...
> **Status:** pre-release, active development. This repository is private until the project reaches release readiness (~Session 18 of the internal build plan). The database foundation (`audtheia/storage/schema.sql`) is complete and verified.
...
- **Marine:** Aquarian H2d hydrophone element + preamp → USB audio ADC. **Terrestrial:** standard microphone + BirdNET.
...
[Audtheia V1](https://audtheiaofficial.github.io/audtheia-environmental-monitoring) remains live, and its repository — [`AudtheiaOfficial/audtheia-environmental-monitoring`](https://github.com/AudtheiaOfficial/audtheia-environmental-monitoring) — is unchanged. **V2 is a new repository.** The two are cross-linked: this README points back to V1, and V1's README points forward to V2 once V2 reaches public release.

The V1 Porifera classifier (RF-DETR Medium, v12; 16,915 images; 126 sponge species; CC BY 4.0) carries forward into V2 as the default **desktop verification model**. The V2 Pi screening model is a YOLO11 trained on the same target species and compiled to `.hef` for the Hailo NPU.

### Pending corrections to V1's published Zenodo record

Two known errors in V1's public Zenodo record have not yet been corrected and **must not be reproduced in V2's own release metadata**:

1. The Porifera classifier (v12) is misattributed as YOLOv11 in the Zenodo record — the correct architecture is **RF-DETR Medium**.
2. The Zenodo description contains emojis, which should be removed for a clean academic record.

## License

MIT — © 2026 Andy Portalatin. See [`LICENSE`](LICENSE).
